...
import { isCellEmpty, makeMove, checkWinner, checkDraw, getBotMove, isPlayerTurn } from "../../game/gameLogic";
...
function applyPlayerMove(game: Game, row: number, col: number, userId: number): MoveResult {
    ...
    if (immediateWinner) {
        ...
        statusText = game.mode === "pve"
            ? MESSAGES.YOU_WIN
            : MESSAGES.SYMBOL_WINS(immediateWinner);
